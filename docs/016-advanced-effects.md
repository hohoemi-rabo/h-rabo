...
export default function CustomCursor() {
  ...
  return (
    <>
      ...
      {/* トレイルカーソル */}
      <motion.div
        className="border-neon-blue pointer-events-none fixed z-40 h-8 w-8 rounded-full border"
        animate={{
...
export default function GlitchText({ text, glitchIntensity = 3, className = '' }: GlitchTextProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      for (let i = 0; i < glitchIntensity; i++) {
        ...
        newText =
          newText.substring(0, randomIndex) + randomChar + newText.substring(randomIndex + 1)
      }
  ...
  return (
    ...
    >
      ...
      {isGlitching && (
        <motion.span
          className="from-neon-blue/20 to-neon-purple/20 absolute -inset-1 bg-gradient-to-r"
          animate={{
            scaleX: [0, 1, 0],
            opacity: [0, 0.8, 0],
          }}
...
export default function MagneticButton({
  ...
  return (
    ...
    >
      ...
      {isHovered && (
        <motion.div
          className="bg-gradient-radial from-neon-blue/20 absolute inset-0 to-transparent"
...
export function PerformanceIndicator() {
  ...
  return (
    <div className="bg-dark-900/90 fixed bottom-4 right-4 z-50 rounded p-2 font-mono text-sm text-white">
      <div>FPS: {fps}</div>
