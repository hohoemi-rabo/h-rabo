...
export const Dynamic3DScene = dynamic(() => import('@/components/3d/Scene'), {
  loading: () => <Loading />,
  ssr: false,
})
...
export const DynamicParticleNetwork = dynamic(
  ...
  {
    loading: () => <div className="bg-dark-900 h-full" />,
    ssr: false,
  }
)
...
export const DynamicContactForm = dynamic(() => import('@/components/forms/ContactForm'), {
  loading: () => <div className="bg-dark-800 h-96 animate-pulse rounded-lg" />,
})
```
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // 基本ページ
  const routes = ['', '/about', '/services', '/blog', '/contact', '/faq', '/privacy'].map(
    (route) => ({
      url: `${baseUrl}${route}`,
