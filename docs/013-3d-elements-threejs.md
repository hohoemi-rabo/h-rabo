...
export default function Scene({ children, camera = { position: [0, 0, 5], fov: 75 } }: SceneProps) {
  return (
  ...
  return (
    <group ref={groupRef}>
      {objects.map((obj, index) => (
        <Float key={index} speed={2 + Math.random() * 2} rotationIntensity={0.5} floatIntensity={2}>
...
export default function InteractiveObject({ position, text, onClick }: InteractiveObjectProps) {
